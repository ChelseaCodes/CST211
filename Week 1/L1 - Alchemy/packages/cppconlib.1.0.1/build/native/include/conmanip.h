...
namespace conmanip
{
   ...
   namespace _details
   {
      ...
      void _setpos(HANDLE const console, COORD const pos)
      {
	   ...
	   //Chelsea trying to add shit without breaking it :D 
	   void _setwinsize(HANDLE const console, bool bAbsolute, const SMALL_RECT * lp_rect)
      {
		 ::SetConsoleWindowInfo(console, bAbsolute, lp_rect); 
      }
   }
   ...
   public:
      ...
      void setpos(int const x, int const y)
      {
	   ...
	   //Chelsea adding things 
	  void setwinsize(int index_x, int index_y, int width, int height)
      {
		  COORD buffer_size = { width + 1, height + 1 };
		  SMALL_RECT smol = { index_x, index_y, width, height }; 
		  _details::_setwinsize(context.handle, TRUE, &smol); 
		  SetConsoleScreenBufferSize(context.handle, buffer_size); 
      }
