...
public:
	...
	void displayDegreeCount(void (*visit)(V)); 
...
*	weight - weight of edge to create edge
* Postcondition:
...
template <typename V, typename E>
void Graph<V, E>::displayDegreeCount(void (*visit)(V))
{
	Vertex<V, E> * v = nullptr;
	for(list<Vertex<V, E>>::iterator it = m_vertices.begin(); it != m_vertices.end(); ++it)
	{
		v = &(*it); 
		visit(v->getData()); 
		std::cout << "Degree: " << v->getDegree() << std::endl << std::endl;
	}
